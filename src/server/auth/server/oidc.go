...
// It also checks that "pachyderm" is in the audience and the token is not
// expired or being used before it's valid.
// TODO: Cache JWKS response
// TODO: Configurable audience
func (o *InternalOIDCProvider) ValidateJWT(token string) (string, error) {
	...
	if claims.ExpiresAt < time.Now().Unix() {
		return "", fmt.Errorf("token has exp %v", claims.ExpiresAt)
	}

	if claims.NotBefore > time.Now().Unix() {
		return "", fmt.Errorf("token has nbf %v", claims.NotBefore)
	}

	if audString, ok := claims.Audience.(string); ok && audString == "pachyderm" {
		return claims.Subject, nil
	}

	if audiences, ok := claims.Audience.([]string); ok {
	...
	return "", fmt.Errorf("token audience didn't include Pachyderm: %v", claims.Audience)
}
